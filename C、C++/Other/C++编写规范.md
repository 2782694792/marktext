...
### 1-3：较长语句（>80字符）分多行书写，长表达式要在低优先级操作符处划分新行，操作符放在新行之首，划分出的新行要适当缩进
...
### 1-4：循环、判断等语句中若有较长的表达式或语句，要进行适应划分，长表达式要在低优先级操作符处划分新行，操作符放在新行之首
...
### 1-5：if、for、do、while、case、switch、default等语句自占一行，且if、for、do、while等的执行语句部分要加括号{ }
...
### 1-7：在两个以上关键字、变量、常量对等操作时，之间的操作符之前、之后或前后加空格；非对等操作时，关系密切的立即操作符（如－>）后不加空格
...
### 2-1：说明性文件（.h文件、.inc文件、.def文件、编译说明文件.cfg等）头部应注释，必须列出：版权说明、版本号、生成日期、作者、内容、功能、与其它文件的关系、修改日志等，还应有函数功能简要说明。
...
### 2-2：源文件头部应注释，列出：版权说明、版本号、生成日期、作者、模块目的/功能、主要函数及其功能、修改日志等。
...
### 2-3：函数头部应注释，列出：函数的目的/功能、输入参数、输出参数、返回值、调用关系（函数、表）等。
...
### 2-4：数据结构声明(数组、结构、类、枚举等)，如果其命名不充分自注释则须加以注释。对数据结构的注释应放其上方；结构中每个域的注释放右方
...
### 2-5：全局变量要有详细注释，包括对其功能、取值范围、哪些函数或过程存取它以及存取时注意事项等的说明
...
### 2-8：在程序块的结束行右方加注释标记，以表明某程序块的结束
...
### 3-1：较短单词通过去掉“元音”形成缩写；较长单词取头几个字母形成缩写；一些有公认缩写
...
### 3-3：禁止取单字符（i、j、k），有具体含义外，还能表明变量类型、数据类型等，可作局部循环变量
...
### 3-4：不用大小写与下划线混排，用作特殊标识如标识成员或全局变量的 m_ 和 g_ ，后加大小写混排的方式
...
### 4-1：避免用不易理解的数字，用标识替代。涉及物理状态或含有物理意义的常量，用有意义的枚举或宏代替
...
### 5-5：仔细设计结构中元素的布局与排列顺序，使结构容易理解、节省占用空间，并减少引起误用现象
...
### 6-1：编写可重入函数应注意局部变量的使用（如使用auto即缺省态局部变量或寄存器变量）

> 说明：编写 C/C++语言的可重入函数时，不应使用 static 局部变量，否则必须经过特殊处理，才能使函数具有可重入性。

### 6-2：编写可重入函数时，若使用全局变量，则应通过关中断、信号量（即P、V操作）等手段加以保护
...
### 6-8：使用动宾词组为执行某操作的函数命名。如果是OOP方法，可只有动词（名词是对象本身）
...
### 7-2：断言检查程序正常运行时不应发生但在调测时有可能发生的非法情况

### 7-3：不能用断言来检查最终产品肯定会出现且必须处理的错误情况
...
### 7-5：用断言对程序开发环境（OS/Hardware）的假设进行检查
...
### 7-6：正式软件产品中把断言及其它调测代码去掉

> 加快软件运行速度

### 7-7：编写防错程序，然后在处理错误之后可用断言宣布发生错误
